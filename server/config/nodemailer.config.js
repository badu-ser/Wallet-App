...
const send = async ({ to, firstName, verificationToken }) => {
  ...
  let body = `<div style="background-color: #F5F5F5; font-family: 'Inter', system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif; font-size: 16px; line-height: 1.5; color: #333333; margin: 0; padding: 50px;">
      <div style="text-align: center;"><a href="${process.env.FRONTEND_DEPLOY_URL}" style="display: inline-block; font-size: 3em; font-weight: bold; color: #007BFF; text-decoration: none;">X4 Wallet</a></div>
      <h2 style="font-size: 1.5em; margin-top: 0; margin-bottom: 0.5em;">Dear ${capitalizedFirstName ?? 'User'},</h2>
      <p style="margin-top: 0;">Thank you for registering in <a href="${process.env.FRONTEND_DEPLOY_URL}" style="color: #007BFF; text-decoration: none;">X4 Wallet</a>.</p>
      ...
      <p style="font-size: 0.8em; margin-top: 0;">Team X4 eSports India 🇮🇳</p>
    </div>`;
  ...
  const emailOptions = {
    ...
    to,
    subject: 'E-mail verification - X4 Wallet',
    html: body,
  };
